...
import { Plus, Trash2, FileText, Download, Printer, Save } from "lucide-react";
...
const DRAFT_PO_STORAGE_KEY = "draft_purchase_orders";

function loadDraftPOs(): DraftPO[] {
  try {
    const stored = localStorage.getItem(DRAFT_PO_STORAGE_KEY);
...
function saveDraftPOToStorage(draft: DraftPO): void {
  const drafts = loadDraftPOs();
  ...
  localStorage.setItem(DRAFT_PO_STORAGE_KEY, JSON.stringify(drafts));
}

function deleteDraftPOFromStorage(id: string): void {
  const drafts = loadDraftPOs().filter(d => d.id !== id);
  localStorage.setItem(DRAFT_PO_STORAGE_KEY, JSON.stringify(drafts));
}
...
export const IssuePODialog = ({ onPOCreated }: IssuePODialogProps) => {
  ...
  const [savedDrafts, setSavedDrafts] = useState<DraftPO[]>([]);
  ...
  const handleSaveDraft = () => {
    ...
    saveDraftPOToStorage(draft);
    setDraftId(id);
    setSavedDrafts(loadDraftPOs());
    
    toast({
      title: "Draft Saved",
      description: "Purchase order draft has been saved",
    });
  };

  const handleLoadDraft = (draft: DraftPO) => {
    setDraftId(draft.id);
    setSelectedVendor(draft.vendorId);
    setLineItems(draft.lineItems.length > 0 ? draft.lineItems : [createEmptyLineItem()]);
    setAdditionalCosts(draft.additionalCosts);
    ...
    toast({
      title: "Draft Loaded",
      description: "Purchase order draft has been loaded",
    });
  };

  const handleDeleteDraft = (id: string) => {
    deleteDraftPOFromStorage(id);
    setSavedDrafts(loadDraftPOs());
    if (draftId === id) {
      setDraftId(null);
    }
    toast({
      title: "Draft Deleted",
      description: "Purchase order draft has been deleted",
    });
  };
            ...
            <div className="space-y-4 pr-4">
                    ...
                    {savedDrafts.map((draft) => (
                      <div
                        ...
                        className={`flex items-center gap-1 px-3 py-1.5 border rounded-lg text-sm ${
                          draftId === draft.id ? "bg-primary/10 border-primary" : "bg-muted/30"
                        }`}
                      >
                        <button
                          type="button"
                        ...
                        >
                          {draft.vendorName || "Untitled"} ({new Date(draft.savedAt).toLocaleDateString()})
                        </button>
                        <Button
                          type="button"
                          variant="ghost"
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-lg">
                  <div className="p-4 space-y-3 max-h-[300px] overflow-y-auto">
                    {/* Header */}
                    <div className="grid grid-cols-12 gap-2 text-xs font-medium text-muted-foreground px-1 sticky top-0 bg-background pb-2">
              ...
              <div className="space-y-2">
                ...
                {additionalCosts.length > 0 && (
                  <div className="space-y-2 p-3 border rounded-lg bg-muted/30 max-h-[200px] overflow-y-auto">
              ...
              {/* Actions */}
              <div className="flex justify-end gap-2 pt-2 border-t sticky bottom-0 bg-background pb-2">
