  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {[1, 2].map(i => <Card key={i} className="mb-4"><CardContent className="pt-6"><Skeleton className="h-24 w-full" /></CardContent></Card>)}
      </div>
    </div>
  );
}
...
function ExpensesContent() {
  ...
  const fetchExpenses = useCallback(async () => getExpenses(), []);

  const { data: expenses, loading, refresh } = useAsyncData(fetchExpenses, {
    loadOnMount: true,
  ...
  const filteredExpenses = q ? allExpenses.filter(expense =>
    expense.employeeName.toLowerCase().includes(q) ||
  ...
  const handleExpenseClick = (expense: Expense) => { setSelectedExpense(expense); setDetailDialogOpen(true); };
  const handleExpenseUpdated = () => { refresh(); setDetailDialogOpen(false); };
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="border-b bg-card">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setAddDialogOpen(true)}><Plus className="h-4 w-4 mr-2" />Add Expense</Button>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="space-y-4">
          {employeeStats.map(({ name, expenses, totalAmount, count }) => (
            <Card key={name}>
              <Collapsible open={employeeExpanded[name]} onOpenChange={(open) => setEmployeeExpanded({ ...employeeExpanded, [name]: open })}>
                          ...
                          <div key={expense.id}
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {expense.jobTitle && <Badge variant="secondary" className="text-xs capitalize">{expense.jobTitle}</Badge>}
                                <span className="text-sm text-muted-foreground">{new Date(expense.expenseDate).toLocaleDateString()}</span>
                              </div>
                              {expense.description && <p className="text-sm text-muted-foreground">{expense.description}</p>}
                              {expense.creditCardLast4 && <p className="text-xs text-muted-foreground mt-1">Card ending in {expense.creditCardLast4}</p>}
                            </div>
                            <div className="text-right ml-4">
                              <div className="text-lg font-semibold text-orange-600">${expense.amount.toFixed(2)}</div>
                              {expense.receiptUrl && <Badge variant="secondary" className="text-xs">Has Receipt</Badge>}
                            </div>
                          </div>
                        ))}
                    </div>
          ...
          {employeeStats.length === 0 && (
            <Card>
              <CardContent>
                <EmptyState
                  icon={Receipt}
                  title={debouncedQuery ? "No expenses match your search" : "No expenses found"}
                  description={debouncedQuery ? "Try a different search term" : "Add your first expense to start tracking"}
                  actionLabel={!debouncedQuery ? "Add Expense" : undefined}
                  onAction={!debouncedQuery ? () => setAddDialogOpen(true) : undefined}
                />
              </CardContent>
            </Card>
          )}
      ...
      <AddExpenseDialog open={addDialogOpen} onOpenChange={setAddDialogOpen} onExpenseAdded={refresh} />
      {selectedExpense && (
        <ExpenseDetailDialog
          expense={selectedExpense} open={detailDialogOpen} onOpenChange={setDetailDialogOpen}
          onExpenseUpdated={handleExpenseUpdated} onExpenseDeleted={refresh}
        />
      )}
    </div>
  );
